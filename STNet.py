...
def STNet(img, batch_size, image_size, num_class = 8, dropout_keep_prob = 0.7, stn_init=np.array([0.7, 0.0, -0.2, 0.0, 0.7, -0.2, 0.0, 0.0]).astype('float32'),
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        net = slim.conv2d(img, 32, [7, 7], stride = 2, scope = 'conv1', weights_initializer = tf.contrib.layers.xavier_initializer_conv2d(),
                          biases_initializer=tf.zeros_initializer())

        net = slim.max_pool2d(net, [2, 2], [2, 2], padding='VALID', scope='pool1')

        net = slim.conv2d(net, 64, [5, 5], stride = 2, scope = 'conv2', weights_initializer = tf.contrib.layers.xavier_initializer_conv2d(),
                          biases_initializer=tf.zeros_initializer())

        net = slim.max_pool2d(net, [2, 2], [2, 2], padding='VALID', scope='pool2')

        net = slim.conv2d(net, 128, [3, 3], stride=2, scope='conv3', weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
                          biases_initializer=tf.zeros_initializer())
        ...
        net = slim.dropout(net, dropout_keep_prob, is_training=False, scope='fc_2_dropout')
        outputs = tf.reshape(stl.transform(img, net), [batch_size, image_size, image_size, 3])
