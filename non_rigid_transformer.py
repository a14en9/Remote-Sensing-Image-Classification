"""
Implementation of rigid and non-rigid transformation
...
class AffineVolumeTransformer(object):
    ...
    def transform(self, inp, theta):
        ...
        with tf.variable_scope(self.name):
            ...
            batch_size, _, _, _, num_channels = tf.shape(inp)[0], tf.shape(inp)[1],tf.shape(inp)[2],tf.shape(inp)[3], tf.shape(inp)[4]
            output = tf.reshape(output,
                                [batch_size, self.out_size[0], self.out_size[1], self.out_size[2], num_channels])
...
class AffineTransformer(object):
    ...
    def transform(self, inp, theta):
        ...
        with tf.variable_scope(self.name):
            ...
            batch_size, _, _, num_channels = tf.shape(inp)[0], tf.shape(inp)[1],tf.shape(inp)[2],tf.shape(inp)[3]
...
class ElasticTransformer(object):
    ...
    def __init__(self, out_size, param_dim=2 * 16, name='SpatialElasticTransformer', interp_method='bilinear',
                 **kwargs):
        """
        Parameters
        ----------
...
def _interpolate(im, x, y, out_size, method):
    out_size = out_size # out_size control
    if method == 'bilinear':
        return bilinear_interp(im, x, y, out_size*3)
    if method == 'bicubic':
        return bicubic_interp(im, x, y, out_size*3) 
    return None
...
def bilinear_interp3d(vol, x, y, z, out_size, edge_size=1):
    with tf.variable_scope('bilinear_interp3d'):
        batch_size, depth, height, width, channels = tf.shape(vol)[0],tf.shape(vol)[1],tf.shape(vol)[2],tf.shape(vol)[3],tf.shape(vol)[4]
...
def bilinear_interp(im, x, y, out_size):
    with tf.variable_scope('bilinear_interp'):
        batch_size, height, width, channels = tf.shape(im)[0],tf.shape(im)[1],tf.shape(im)[2],tf.shape(im)[3]
